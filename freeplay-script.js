...
let blockLimit = 11; // Default block limit, will be overridden in Free Play
let isFreePlayMode = true; // Always true for this script
...
// Get average views for each article 
...
async function resetGame(customTopic, blockCount) {
    ...
    // Set dailyTopic to the custom topic
    dailyTopic = customTopic;
    
    // Reset CSV with the custom topic
    ...
    // Create a new initial block with the custom topic
    ...
    // Set the block limit based on user input (blockCount + 1)
    blockLimit = blockCount + 1;
    ...
    if (allBlocks.length === blockLimit) {
        ...
        visitButton.addEventListener('click', () => {
            ...
            analytics.logEvent('game_complete', {
                score: totalScore,
                initial_block: dailyTopic,
                user_id: currentUser ? currentUser.uid : generateUniqueId()
            });  
        });
...
input.addEventListener('keydown', async (e) => {
    if (e.key === 'Enter' && input.value.trim() !== '') {
        ...
        if (allBlocks.length === blockLimit) {
            ...
            // In Free Play mode, do not save to Firestore
            ...
            // No rank in Free Play mode
            const rankText = document.createElement('p');
            rankText.style.margin = '0 0 15px 0';
            rankText.textContent = 'Free Play: No ranking available';
            ...
            resetButton.addEventListener('click', async () => {
                // Prompt for new topic and block count on reset
                showFreePlayPopup();
                popup.remove();
            });
...
function showFreePlayPopup() {
    ...
    popup.querySelector('#start-free-play-button').addEventListener('click', async () => {
        ...
        const blockCount = parseInt(blockCountInput);
        console.log(`Free Play started: Topic = ${customTopic}, Block Limit = ${blockCount + 1} (including initial block), Blocks Left will start at ${blockCount}, isFreePlayMode = ${isFreePlayMode}`);

        // Reset the game with the new settings
        await resetGame(customTopic, blockCount);
...
document.getElementById('free-play-button').addEventListener('click', showFreePlayPopup);
...
document.getElementById('how-to-play-button').addEventListener('click', () => {
    ...
    popup.innerHTML = `
        ...
        <ol style="text-align: left; padding-left: 20px; margin-bottom: 20px;">
            ...
            <li>Add your chosen number of Blocks to complete the game in Free Play.</li>
            <li>The more niche the topic the more points you earn.</li>
            <li>For example, if the Topic Block is <span style="color: #6273B4;">New York City</span>, an acceptable answer would be <span style="color: #6273B4;"> The Empire State Building </span> or the <span style="color: #6273B4;">NYC Marathon</span>.</li>
